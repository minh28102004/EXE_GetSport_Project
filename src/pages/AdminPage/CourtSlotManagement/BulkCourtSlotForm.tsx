...
type Props = {
  onSave: (bulkData: Partial<CourtSlot>) => void | Promise<void>;
  onClose: () => void;
  loading?: boolean;
};
...
const BulkCourtSlotForm: React.FC<Props> = ({ onSave, onClose, loading }) => {
  const [form, setForm] = useState<Partial<CourtSlot>>({
    courtId: 0,
    startTime: "",
    endTime: "",
  });
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="sticky top-0 z-10 -mt-2 -mx-2 flex items-center justify-between rounded-t-xl bg-white/90 px-2 py-2 backdrop-blur border-b">
        <h2 className="text-2xl font-semibold text-slate-900">Create Bulk Slots</h2>
        <button
          type="button"
          onClick={onClose}
      ...
      <Section icon={<Calendar className="w-4 h-4" />} title="Bulk Slot Information">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setField("startTime", e.target.value || undefined)}
              required
              disabled={loading}
            />
          </div>
          <div>
            ...
            <input
              ...
              className={baseField}
              value={form.endTime ?? ""}
              onChange={(e) => setField("endTime", e.target.value || undefined)}
              required
              disabled={loading}
            />
          </div>
...
export default BulkCourtSlotForm;

type ModalProps = Props & { open: boolean };

export const BulkCourtSlotFormModal: React.FC<ModalProps> = ({
  open,
  onClose,
  ...formProps
}) => {
        ...
        >
          ...
          >
            <div className="flex-1 min-h-0 max-h-[80vh] overflow-y-auto p-4">
              <BulkCourtSlotForm {...formProps} onClose={onClose} />
            </div>
          </motion.div>
        </motion.div>
      )}
